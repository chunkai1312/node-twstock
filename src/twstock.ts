...
export class TwStock {
  ...
  get stocks() {
    return {
      ...
      rightsAndDividend: this.fetchStocksRightsAndDividend.bind(this),
    };
  }
  ...
  private async fetchStocksRightsAndDividend(options: { exchange?: 'TWSE' | 'TPEx', startDate: string, endDate: string, symbol?: string }) {
    const { symbol, startDate, endDate } = options;
    ...
    return (exchange === Exchange.TPEx)
      ? await this._scraper.getTpexScraper().fetchStocksRightsAndDividend({ symbol, startDate, endDate })
      : await this._scraper.getTwseScraper().fetchStocksRightsAndDividend({ symbol, startDate, endDate });
  }
