...
export class TaifexScraper extends Scraper {
  ...
  async fetchOptionsHistorical(options: { date: string, symbol: string, afterhours?: boolean }) {
    ...
    const data = rows.map(row => {
      const [date, contract, month, strikePrice, type, ...values] = row;
      const data: Record<string, any> = {};
      data.date = DateTime.fromFormat(date, 'yyyy/MM/dd').toISODate();
      data.symbol = this.buildFutOptContractSymbol(contract, month, { strikePrice, type });
      data.month = month;
  ...
  async fetchFuturesInstTrades(options: { date: string, symbol: string }) {
  ...
  async fetchTxfInstTrades(options: { date: string }) {
    const { date } = options;
    return this.fetchFuturesInstTrades({ date, symbol: 'TXF' })
  }
  ...
  async fetchFuturesLargeTradersPosition(options: { date: string, symbol: string }) {
    const { date, symbol } = options;
    ...
    const txRows = rows.filter(row => row[1] === (alias[symbol] ?? symbol));
